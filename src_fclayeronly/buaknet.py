...
class Layer(ABC):
    ...
    def load_params(self, network_name, layer_name, w_shape, b_shape):
        current_dir = os.getcwd()
        print(current_dir)
        network_dir = current_dir + "//szakdoga//3as fejezet kódja//" + network_name
        os.chdir(network_dir)
    ...
    def update_params(self, delta_nabla_w, delta_nabla_b, traning_number, minibatch_size, learning_rate):
        self.weights =  self.weights - (learning_rate / minibatch_size) * delta_nabla_w
...
class FullyConnectedLayer(Layer):
    ...
    def backward(self, minibatch_delta, previouslayer, traning_number, minibatch_size, learning_rate):
        ...
        for j in range(epochs):
            ...
            for minibatch in mini_batches:
                ...
                minibatch_delta = self.layers[-1].outputerror(self.layers[-2], out_minibatch, traning_number,
                                                              minibatch_size, learning_rate, costfn)
