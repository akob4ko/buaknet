...
class Atrament {
  ...
  scaleDown_letter() {
    ...
    mnistCtx.drawImage(originalCtx.canvas, 0, 0);
    mnistCtx.fillRelatedField = function () {
      document.getElementById('data-paint_letter').value = mnistCanvas.toDataURL();
    };
    mnistCtx.fillRelatedField()
  }
...
var sketcher_letter = new Atrament('#original_letter',250,250)
$('#post_data_letter').on('click', (e) => {
  const mnistCanvas = document.getElementById('mnist_letter');
  mnistCanvas.style.border ="1px solid #000000";
  sketcher_letter.scaleDown_letter();
});
$('#clear_letter').on('click', (e) => {
  const mnistCanvas = document.getElementById('mnist_letter');
  mnistCanvas.style.border ="0px solid #000000";
  sketcher_letter.clear_letter();
  sketcher_letter.scaleDown_letter();
});
...
$('a#post_data_letter').bind('click', function() {
            $.ajax({
                url : $root + '/get_letter',
                type: "POST",
                data: JSON.stringify([
                    {digit: $('textarea[id="data-paint_letter"]').val()}
                ]),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function(data){
                    $("#res_letter").append("<span style=\"color: #669bf6; font-size: 20pt;\">"+data.result+"</span>");
                      sketcher_letter.clear_original_letter();
                }
            });
            return false;
        });
